...
cd $basedir
tar zxf ipvsadm-1.24.tar.gz -C $install_tmp/ipvsadm-1.24
cd $install_tmp/ipvsadm-1.24
make && make install

cd $basedir
tar zxf keepalived-1.1.17.tar.gz -C $install_tmp/keepalived-1.1.17
cd $install_tmp/keepalived-1.1.17
./configure && make && make install

\cp $base_config_dir/keepalived.conf /usr/local/etc/keepalived/
